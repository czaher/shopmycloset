...
export default function AdminClient({
  ...
  async function uploadFile(file: File): Promise<string | null> {
    ...
    setForm((p) => ({
      ...p,
      uploading: p.uploading.filter((id) => id !== tempId),
    }))
    ...
    <div className='min-h-screen bg-cream'>
      <header className='border-b border-warm-beige bg-white sticky top-0 z-10'>
        <div className='max-w-5xl mx-auto px-6 py-4 flex items-center justify-between'>
          <h1 className='font-serif text-xl text-warm-brown'>
            admin zone (⊙_◎)
          </h1>
          <div className='flex items-center gap-4'>
            ...
            <button
              ...
              className='text-sm text-muted-brown hover:text-warm-brown transition-colors'
            >
              Sign out
            </button>
          </div>
              ...
              >
                ...
                <div className='sm:col-span-2'>
                  ...
                  >
                    {isUploading ? (
                      <p className='text-sm text-muted-brown'>
                        Uploading {form.uploading.length}{' '}
                        {form.uploading.length === 1 ? 'photo' : 'photos'}...
                      </p>
                    ) : (
                      <>
                        <svg
                          className='w-8 h-8 text-muted-brown/60'
                          fill='none'
                          stroke='currentColor'
                          viewBox='0 0 24 24'
                        >
                        ...
                        <p className='text-sm text-muted-brown'>
                          Drop photos here or{' '}
                          <span className='text-terra underline'>
                            click to browse
                          </span>
                        </p>
                        <p className='text-xs text-muted-brown/60'>
                          JPG, PNG, WEBP · multiple at once is fine
                        </p>
                      </>
                    )}
                  </div>
                ...
                {formError && (
                  <p className='sm:col-span-2 text-red-500 text-sm'>
                    {formError}
                  </p>
                )}
            ...
            {claims.length === 0 ? (
              ...
              <div className='flex flex-col gap-3'>
                {claims.map((claim) => (
                  ...
                  >
                    <div>
                      <p className='font-medium text-warm-brown text-sm'>
                        {claim.claimer_name}
                      </p>
